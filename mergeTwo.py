...
import json
from Get_list_from_ref import list_reference, list_id
from get_list_from_whis import wordtimestamp, whisper
...
merge_result = []
for i, word in enumerate(nw_ref_match_flags(list_reference, whisper)):
    if word[1] == 1:
        merge_result.append({"word": word[0], "idx_w": list_id[i][1][0], "idx_s": list_id[i][1][1], "has_timestamp": True, "start": wordtimestamp[word[2]]["start"], "end": wordtimestamp[word[2]]["end"]})

    else:
        merge_result.append({"word": word[0], "idx_w": list_id[i][1][0], "idx_s": list_id[i][1][1],"has_timestamp": False, "start": None, "end": None})
